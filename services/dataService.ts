...
export const dataService = {
  ...
  async getProperties(userId: string, folderId?: string | null) {
    ...
    return (data || []).map((p: any) => ({
      ...
      fees: Number(p.fees),
      feesCurrency: p.fees_currency || 'ARS',
  ...
  async createProperty(property: Partial<Property>, userId: string) {
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        fees: property.fees,
        fees_currency: property.feesCurrency || 'ARS',
  ...
  async updateProperty(id: string, property: Partial<Property>) {
    const { data, error } = await supabase
      ...
      .update({
        ...
        fees: property.fees,
        fees_currency: property.feesCurrency,
        environments: property.environments,
        rooms: property.rooms,
        bathrooms: property.bathrooms,
        toilets: property.toilets,
  ...
  async getSharedItinerary(id: string) {
    ...
    const { data: itinerary, error: itinError } = await supabase
      .from('shared_itineraries')
      .select('*, folder:folders(user_id, name, description, color, budget, transaction_type, start_date, status, welcome_message, image_url, is_image_public, client_id, client:clients(name))')
      .eq('id', id)
      .eq('is_active', true)
      .single();
