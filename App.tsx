    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className="flex-1 p-10 overflow-y-auto custom-scrollbar">
        ...
        {activeTab === 'properties' && (
          <div className="mb-8 flex flex-wrap items-center justify-between gap-6 bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="flex flex-1 min-w-[300px] relative">
              <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <input 
                type="text"
                placeholder="Buscar por título o dirección..."
                className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-3.5 pl-12 pr-10 text-xs font-bold text-slate-700 placeholder:text-slate-300 outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all"
                value={searchQuery}
            ...
            <div className="flex items-center gap-4">
              <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200">
                {(['All', ...Object.values(PropertyStatus)] as string[]).map((status) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${statusFilter === status ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
                  >
              ...
              <div className="relative group">
                <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 px-5 py-3 rounded-2xl text-[9px] font-black text-slate-500 uppercase tracking-widest cursor-pointer group-hover:border-indigo-200 transition-all">
