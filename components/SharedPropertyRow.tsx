  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-3 flex flex-col md:flex-row gap-4 items-center hover:shadow-md transition-all group">
      ...
      <div className="relative w-full md:w-40 h-32 md:h-24 shrink-0 rounded-xl overflow-hidden group-hover:brightness-110 transition-all cursor-pointer" onClick={() => onSelect(property)}>
        <div className="absolute top-2 left-2 z-10 bg-slate-900/80 text-white w-5 h-5 rounded-md flex items-center justify-center font-bold text-xs backdrop-blur-sm">
      ...
      <div className="flex-1 min-w-0 w-full">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-2">
             <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${getStatusColor(property.status)}`}>
              {property.status}
            </span>
            <span className="bg-slate-100 px-2 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 text-slate-600">
              {ICONS.Star} {property.rating}
            </span>
            {property.fees && property.fees > 0 && (
                <span className="flex items-center gap-1 text-[10px] text-amber-600 font-bold uppercase tracking-wider bg-amber-50 px-2 py-0.5 rounded-full">
        ...
        {property.clientCustomFields && Object.keys(property.clientCustomFields).length > 0 && (
          <div className="flex flex-wrap items-center gap-2 mb-2">
            {Object.entries(property.clientCustomFields).map(([key, value]) => (
              <span key={key} className="bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 border border-indigo-100">
      ...
      {/* Price & Actions */}
      <div className="flex flex-col items-end gap-3 w-full md:w-auto justify-between md:justify-center border-t md:border-t-0 md:border-l border-slate-100 pt-3 md:pt-0 md:pl-4">
        <div className="text-right w-full flex justify-between md:block items-center">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 w-full">
            <button
                ...
                className="flex-1 md:flex-none bg-white border border-dashed border-indigo-200 text-indigo-500 hover:bg-indigo-50 hover:border-indigo-300 px-3 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-colors flex items-center justify-center gap-2"
            ...
            >
             ...
             <button 
                ...
                className={`flex-1 md:flex-none px-3 py-2 rounded-xl border transition-all flex items-center justify-center gap-2 text-[10px] font-bold uppercase tracking-wider ${isCompared ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-slate-200 text-slate-400 hover:border-indigo-200 hover:text-indigo-600'}`}
                title="Comparar"
            >
             ...
             <button 
                ...
                className="flex-1 md:flex-none bg-white border border-slate-200 text-slate-600 px-3 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider hover:bg-slate-50 hover:text-indigo-600 transition-all flex items-center justify-center gap-2"
                title="Ver Ficha"
            >
                <ExternalLink className="w-3.5 h-3.5" />
            ...
            <button 
                ...
                className="flex-1 md:flex-none bg-indigo-600 text-white px-3 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-md shadow-indigo-200"
                title="Pedir Visita"
            >
