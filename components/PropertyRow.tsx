...
const PropertyRow: React.FC<PropertyRowProps> = ({ property, index, onSelect, onStatusChange, onToggleVisibility, onEdit, onDelete, isEditable = true }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-3 flex flex-col md:flex-row gap-4 items-center hover:shadow-md transition-all group">
      {/* Image & Index */}
      <div className="relative w-full md:w-40 h-32 md:h-24 shrink-0 rounded-xl overflow-hidden group-hover:brightness-110 transition-all">
        ...
        <div className="flex items-center gap-2">
             ...
             <div className="flex gap-1">
                {onToggleVisibility && (
                    ...
                    className={`p-1.5 rounded-lg transition-all ${property.isPublic !== false ? 'text-slate-400 hover:text-indigo-600 hover:bg-indigo-50' : 'text-slate-300 hover:text-slate-500'}`}
                    title={property.isPublic !== false ? "Visible" : "Oculto"}
                    >
