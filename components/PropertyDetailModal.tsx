...
const PropertyDetailModal: React.FC<PropertyDetailModalProps> = ({ property, onClose, userRole, onUpdateReno }) => {
  ...
  const isIframeBlocked = (url: string) => {
    if (!url) return false;
    const blocked = ['remax', 'idealista', 'zillow', 'fotocasa', 'arbol', 'zonaprop', 'mercadolibre', 'portalinmobiliario', 'argenprop', 'inmuebles24', 'finca_raiz', 'tokkobroker', 'properati', 'habitaclia'];
        ...
        <div className="flex-1 bg-slate-900 h-full relative flex flex-col overflow-hidden">
          ...
          <div className="p-4 bg-white/5 border-b border-white/5 flex items-center justify-between backdrop-blur-md z-30">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => {setSnapshotVersion(Date.now()); setSnapshotLoading(true); setSnapshotError(false);}} className="p-2 text-slate-400 hover:text-white transition-all">
                <RefreshCw className={`w-4 h-4 ${snapshotLoading && activeRefTab === 'snapshot' ? 'animate-spin' : ''}`} />
              </button>
              ...
              <div className="w-full h-full relative overflow-auto custom-scrollbar flex items-center justify-center p-8 bg-slate-100">
                ...
                {snapshotError ? (
                  <div className="text-center max-w-sm">
                    ...
                    <p className="text-slate-400 text-sm mb-6">This portal blocks automated AI captures. Use the <b>Live Portal</b> tab to try embedding.</p>
                  </div>
                ) : (
                  <img 
                    key={snapshotVersion}
                    src={`https://s.wordpress.com/mshots/v1/${encodeURIComponent(property.url)}?w=1600&v=${snapshotVersion}`}
                ...
                {isIframeBlocked(property.url) && (
                  <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-12 text-center">
                    <div className="max-w-md">
                      ...
                      <h4 className="text-2xl font-black text-white mb-2">Embed Shield Active</h4>
                      <p className="text-slate-400 text-sm leading-relaxed mb-8">
                        Este portal (RE/MAX, Idealista, etc.) bloquea la visualización dentro de otras apps. 
                        Utiliza <b>AI Snapshot</b> o abre el sitio externamente.
                      </p>
                      <button onClick={() => setActiveRefTab('snapshot')} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-indigo-500 transition-all">Switch to Snapshot</button>
                    </div>
                  </div>
                )}
              </div>
